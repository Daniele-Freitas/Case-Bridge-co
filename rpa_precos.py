...
class PrecosReferenciaError(RuntimeError):
    pass
...
def _baixar_html(opts: FetchOptions) -> str:
    ...
    # resp.apparent_encoding pode ajudar em alguns sites; aqui mantemos simples
    return resp.text
...
def extrair_precos_referencia(
    ...
    *,
    ...
    """Extrai a tabela de preços de referência a partir de uma página HTML.

    Retorna um DataFrame com a tabela encontrada no HTML (primeira tabela por padrão).
    """
    ...
    # Normalização leve: remove espaços extras nos nomes das colunas
...
def extrair_precos_referencia_de_arquivo(
    ...
    *,
    ...
    """Variante offline: extrai a tabela a partir de um arquivo .html local."""
    path = Path(html_path)
    if not path.exists():
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.html:
        ...
        df = extrair_precos_referencia(
            args.url, table_index=args.table_index, timeout_s=20.0
        )
